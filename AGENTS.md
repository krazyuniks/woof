...
- Preserve the interactive harness transport boundary (ADR-012). Profiles select tmux or herdr explicitly; do not add headless `claude -p`, `codex exec`, or equivalent one-shot reasoning paths. Resolve the transport from the profile at the transport seam; never infer it from a harness name or a command name.
- Preserve Woof's ownership of its herdr named session (ADR-018). The session is declared through `WOOF_HERDR_SESSION` and never defaulted; never dispatch a worker into, or tear down, an operator session (`default`, `drains`). Protocol work and live smokes use a disposable named session on its own socket.
