...
- **Worker identity** - the durable, backend-neutral record of a worker: its stable name, the backend's reference to the live worker, and the session, socket, protocol, and version it was reached through. It is written as soon as the worker exists, and survives the client that launched the worker, so a fix round reattaches and a lost worker respawns under the same name. A worker whose record does not survive is still found by its stable name, and so is reattached to or killed rather than orphaned.
...
- **Dead session** - a named session whose socket file exists but has no server answering behind it. Presence of the socket proves nothing, and neither does an accepted connection; only a completed ping is liveness. Death is confirmed by repeated failed probes, because reaping unlinks the socket. Dispatch reaps a dead session's sockets and respawns its server.
