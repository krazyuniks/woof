...
ADR-001 places workflow authority in the Python engine, with a Claude Code operator skill over the CLI. The engine dispatches LLM work as typed roles. Roles are semantic, not provider-named: the dispatch policy chooses the best model per stage without leaking provider names into prompt prose or graph state.
...
Roles are semantic. The engine dispatches by role name; the route picks the adapter.
...
- Codex has no LSP integration. Structural facts a Codex node needs are injected by Woof from cartography or tree-sitter queries.
