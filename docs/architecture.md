...
Herdr compatibility is established against the running named-session server reached through its explicit socket. Preflight records the socket, server version, and protocol, and fails before dispatch when the protocol does not match the one this transport is built against. Liveness is an accepted connection, never the presence of the socket file: a socket file with no listener behind it is a dead session, and dispatch reaps its leftover sockets and respawns the server rather than failing forever with a refused connection.
