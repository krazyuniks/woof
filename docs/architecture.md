...
Worker identity is durable and backend-neutral. It is recorded under `~/.woof/state/projects/<project-key>/epics/<epic-id>/runs/workers/<worker-name>.json` and holds the backend, the stable worker name, the backend's worker reference, and the session, socket, protocol, and version the worker was reached through. Disk is the authority: a turn reattaches to the recorded worker when it is still alive and respawns it under the same name when it is not. The producer's worker name is derived from run id, work-unit id, and role, so it is stable across fix rounds and across a client restart; each reviewer round gets its own name and its worker is torn down when the round ends. `woof dispatch --close-worker` terminates a work unit's retained producer and clears its record, so a worker cannot outlive the run that launched it.
