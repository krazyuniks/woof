...
7. `commit` computes the transaction manifest, stages the exact expected file set, verifies the index, appends graph events, commits with `executor_result.commit_subject` when supplied, and removes transient `executor_result.json` / `check-result.json`.
...
plan critique. Transient result files, gates, locks, sync markers, and
...
| `woof observe --epic <N> --view status\|timeline\|gate\|audit` | Inspect local workflow status, merged event timeline, current gate, or audit/usage state without mutating the epic or contacting the tracker. |
