...
The transport is resolved from the selected harness profile at one seam. Workflow code carries no harness name and no transport branch, and no public result or event field is named after a backend: `transport_backend`, `transport_session`, `transport_socket`, `transport_protocol`, `transport_version`, `worker_name`, and `worker_ref` carry either backend.

Worker identity is durable and backend-neutral. It is recorded under `~/.woof/state/projects/<project-key>/epics/<epic-id>/runs/workers/<worker-name>.json` and holds the backend, the stable worker name, the backend's worker reference, and the session, socket, protocol, and version the worker was reached through. A retained worker is recorded as soon as it exists, before its turn is delivered, because it survives a turn that fails. Disk is the authority: a turn reattaches to the recorded worker when it is still alive and respawns it under the same name when it is not. A worker with no identity record is looked up by its stable name before any worker is started, so an orphan is adopted rather than duplicated. The producer's worker name is derived from run id, work-unit id, and role, so it is stable across fix rounds and across a client restart; each reviewer round gets its own name and its worker is torn down when the round ends. `woof dispatch --close-worker` terminates a work unit's retained producer and clears its record, and falls back to the stable name when no record survives, so a worker cannot outlive the run that launched it.

A worker turn fails in three distinct ways, each a separate outcome carrying the worker's own session output as evidence: `blocked` (waiting on human input), `wallclock_timeout` (the turn never settled), and `payload_absent` (the turn settled or the worker exited without writing the artefact it was asked for). Evidence is persisted next to the dispatch audit files rather than only interpolated into an error message.

Herdr compatibility is established against the running named-session server reached through its explicit socket. Preflight records the socket, server version, and protocol, and fails before dispatch when the protocol does not match the one this transport is built against. Liveness is a completed ping, never the presence of the socket file and never a bare accepted connection: a socket file with no server answering behind it is a dead session, and dispatch reaps its leftover sockets and respawns the server rather than failing forever with a refused connection. Death is confirmed by repeated failed probes before the socket is reaped, because unlinking the socket of a server that was merely unreachable would strand it behind a path a second server then binds.

Woof declares and owns the herdr named session it dispatches into (`WOOF_HERDR_SESSION`). There is no implicit default, because the herdr server, not the client, spawns the worker: an assumed session would place Woof's workers inside whichever server happened to be serving. Woof starts no worker in an operator session and tears none down. Protocol work and live smokes use a disposable named session on its own socket (ADR-018).
