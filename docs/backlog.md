...
work_units:
  ...
  - id: vaultforeman-fix-parity
    ...
    state: todo
    priority: high
    summary: "Re-baseline the runner absorption against VaultForeman HEAD. The runner-asset source map was cut 2026-06-28; VaultForeman has landed drain, review-parsing, prompt, and operator-UX fixes since that Woof will not otherwise inherit. Carry the fix families below and refresh the source map to VF HEAD. Two VF changes are deliberately NOT inherited: the state-to-operator-home move (VF ac9ac02) — Woof's disk authority is `.woof/` in the consumer repo (architecture section 1) — and the optional-worktree-lifecycle change (VF 0411fd5), which ADR-015 already supersedes."
    ...
    acceptance:
      ...
      - "Review-verdict parsing is harness-aware: glyphless, Unicode, and settled-chrome Claude Code done-markers reap a PASS; GLM and codex readiness and done glyphs are matched; multi-line and bare file:line findings are captured whole; TUI pane markers are normalised before verdict parsing; a [path:line] citation the reviewer TUI hard-wrapped across rows is rejoined before finding extraction."
      - "Review-verdict classification recovers non-conforming reviewer output: a settled clear-prose conclusion coerces to PASS only when the harness done-marker is present and no findings, blocking prose, or negations exist; unformatted blocker prose never coerces a verdict — it parks UNCERTAIN with the blocker text preserved verbatim for the operator and the full pane history captured in the park artefact. The same recovery applies when the structured-JSON answer contract is not met."
