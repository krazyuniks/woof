...
- S6: baseline freshness and explicit recapture - `quality-gates-baseline.schema.json` gains optional freshness fields: `captured_iteration` (monotonic woof run count at capture, sourced from `.woof/wf-run-count`), `expiry_seconds` (wall-clock expiry from `captured_at`), and `expiry_iterations` (run-count expiry from `captured_iteration`). An expired baseline (past wall-clock OR iteration limit) is ignored: the check runner surfaces a clear "baseline expired" note and suppresses nothing. Baselines without freshness fields retain S5 behaviour (no expiry). `woof baseline capture` is the only path that writes the baseline; it runs all configured gates, records their pass/fail state with freshness metadata, and writes the record. `check_1_quality_gates` never rewrites the baseline.
...
Moved to `docs/CONTEXT.md` - the one durable home for what the words mean.
