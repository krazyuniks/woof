...
Open work:
- Cartography toolchain conformance to ADR-004: a missing `ctags` degrades silently today - `scripts/refresh-cartography` writes a 0-byte `tags` and exits 0, and `preflight.py`'s mechanical-layer check uses `is_file()` only (`CARTOGRAPHY_MECHANICAL_FILES`), so an empty symbol index passes preflight green and the Stage-5 executor runs context-starved. ADR-004 already mandates the correct behaviour ("ctags is a hard prerequisite; preflight verifies the binary is on PATH"; "a script that exits non-zero makes the hook fail loud rather than silently leaving stale mechanical state"); the code diverged. Fix to conform: (a) the composed `refresh-cartography` source - the per-language template under the registry/template path that `woof init` composes, not only the generated `scripts/refresh-cartography` (managed block is overwritten) - exits non-zero with the operator install command when a declared cartography language needs ctags and it is absent, never writing an empty index; (b) `preflight.py` adds a fail-closed `cartography.ctags` finding when `[cartography].languages` is non-empty and `ctags` is not on PATH, the finding text naming the install command; (c) `scripts/first-time-setup.sh` adds ctags to the check-and-instruct (`require_manual_tool`) tier, not an auto-install tier. No agent ever runs `sudo`/auto-install - the operator runs any privileged install. Tests use a real PATH-stripped seam (not `unittest.mock`): refresh exits non-zero with ctags off PATH; the preflight finding fires with ctags off PATH and passes with it present; the setup prerequisite list includes ctags.
