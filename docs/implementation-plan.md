...
Goal:
Phase A of the release-closure audit is complete and Phase B RC-B1 is closed. The next session should pick the first `Ready` workstream in the Phase B (Portability For Arbitrary Consumers) section of docs/implementation-plan.md - at the time this prompt was written that is RC-B2: Issue-Tracker Abstraction. The rule remains: pick a coherent workstream slice large enough to close multiple related child gaps, not a single tiny row. Stop only at a real risky boundary or external blocker.
...
Start with:
The historical phase ledger through Phase 18 `WFL-001`, release-closure workstreams RC-1 through RC-7, and Phase B RC-B1 are all complete. Phase A of the Release-Closure Audit is closed. The next active backlog is Phase B (Portability For Arbitrary Consumers) which still covers BHID-002 (issue-tracker abstraction) and BHID-003 (first-run consumer onboarding); BHID-001 (Stage 1 producer skill bundling) closed under RC-B1 and BHID-004 (`bin/woof` portability) was resolved by RC-6 for the graph-subprocess re-entry path. Start with RC-B2 unless the Phase B workstream table records an explicit dependency that overrides the order.

RC-B1 closed on 2026-05-20 with one commit. Stage 1 Discovery now runs four graph producer nodes - `discovery_research`, `discovery_thinking`, `discovery_brainstorm`, then `discovery_synthesis`. The three bucket nodes live in `src/woof/graph/nodes.py` (`_discovery_bucket_node` plus three thin wrappers) and are wired through `transitions.py` (`discovery_bucket_complete`, `_DISCOVERY_BUCKET_NODES`, the `next_node` Stage-1 walk) and `default_registry()`. `playbooks/discovery/{research,thinking,brainstorm}.md` are the graph-owned bucket producer prompts; the research and thinking nodes embed their building-block playbook text directly. The 20 building-block playbooks under `playbooks/discovery/research/` and `playbooks/discovery/consider/` were rewritten for non-interactive dispatch (`type: discovery-playbook` frontmatter, no `AskUserQuestion`, output into `.woof/epics/E<N>/discovery/<bucket>/`). `ask-me-questions.md` was left as a documented human-operator intake aid. Schemas `node-input`, `node-output`, `planning-node-input`, `planning-node-output`, and `jsonl-events` carry the new node types, `discovery_bucket_input`, and the `discovery_bucket_explored` event.

Phase B sequencing: the Phase B Workstreams table is the authority. RC-B2 introduces ADR-003 (the issue-tracker boundary). RC-B3 depends on RC-B1 and RC-B2; RC-B4 depends on all three. Preserve ADR-001 and ADR-002 invariants: Woof stays graph-led, GPT-5.5 is the preferred primary producer route, Claude Opus 4.7 at `max` effort is the preferred reviewer route, reviewer blockers open human gates, and Woof must not depend on Woof-author-local wrappers or host-specific absolute paths.
```
