...
| RC-B4: Phase B Release Evidence | Ready | — | README, architecture, schemas, CLI help, and examples describe a release surface usable by an arbitrary consumer. PyPI publish path is documented; a release smoke test installs Woof from a built wheel, runs `woof init` in a temporary consumer worktree against the `local` tracker, and confirms Stage 1 producer nodes dispatch without Ryan-local skill assumptions. | Release-smoke evidence under `examples/dogfood/` or `tests/integration/`; docs review; schema validation; `git diff --check`; `just check`. | `docs(release): record phase b portability evidence` |
...
Goal:
Phase A of the release-closure audit is complete and Phase B RC-B1, RC-B2, and RC-B3 are closed. The next session should pick the first `Ready` workstream in the Phase B (Portability For Arbitrary Consumers) section of docs/implementation-plan.md - at the time this prompt was written that is RC-B4: Phase B Release Evidence, the final Phase B workstream. The rule remains: pick a coherent workstream slice large enough to close multiple related child gaps, not a single tiny row. Stop only at a real risky boundary or external blocker.
...
Start with:
The historical phase ledger through Phase 18 `WFL-001`, release-closure workstreams RC-1 through RC-7, and Phase B RC-B1, RC-B2, and RC-B3 are all complete. Phase A of the Release-Closure Audit is closed and all four Phase B hidden gaps are resolved: BHID-001 (Stage 1 producer skill bundling) under RC-B1, BHID-002 (issue-tracker abstraction) under RC-B2, BHID-003 (first-run consumer onboarding) under RC-B3, and BHID-004 (`bin/woof` portability) under RC-6. The only remaining Phase B workstream is RC-B4 (Phase B Release Evidence). Start with RC-B4.

RC-B3 closed on 2026-05-20. `woof init` was already scaffolding `.woof/{prerequisites,agents,quality-gates,test-markers}.toml` plus the `.gitignore` block from RC-5; RC-B3 added `woof init --tracker {github,local}` (default `github`) so a consumer with no hosted issue tracker scaffolds the no-remote `local` tracker. The `local` scaffold omits the `gh` `[infra]` requirement and the `[tracker]` `repo` line and validates against `prerequisites.schema.json` as-is; the `github` scaffold keeps the intended `<replace>` repo placeholder that fails loud at preflight. The cartography decision stayed as taken in RC-5/GAP-017: no default `scripts/refresh-cartography` ships, cartography is consumer-owned, and the post-commit hook block is a no-op when the script is absent. `woof init` next-steps output now reaches `woof wf new` and points at the walkthrough; the result header records the scaffolded tracker. The first-run walkthrough lives in `docs/consumers.md` as an end-to-end eight-step sequence from `uv tool install woof` / `pip install woof` to a running `woof wf --epic <N>`, and README plus `docs/architecture.md` describe `--tracker` and link the walkthrough.

RC-B4 (the final Phase B workstream) records Phase B release evidence: confirm README, architecture, schemas, CLI help, and examples describe a release surface usable by an arbitrary consumer; document the PyPI publish path; and add a release smoke test that installs Woof from a built wheel, runs `woof init --tracker local` in a temporary consumer worktree, and confirms Stage 1 producer nodes dispatch without Woof-author-local skill assumptions. Evidence belongs under `examples/dogfood/` or `tests/integration/`. Preserve ADR-001, ADR-002, and ADR-003 invariants: Woof stays graph-led, GPT-5.5 is the preferred primary producer route, Claude Opus 4.7 at `max` effort is the preferred reviewer route, reviewer blockers open human gates, the issue tracker stays behind the `Tracker` protocol, and Woof must not depend on Woof-author-local wrappers or host-specific absolute paths.
```
