...
Start CC-006 next. CC-004 is blocked until Ryan makes the runtime-permission policy decision; skip it in the automatic continuation loop until then.
...
Status:
The project is under the 2026-05-21 course correction. Ryan's own development use is the urgent priority. Portfolio exemplar value comes after the core loop works. OSS/distribution polish is deferred. CC-001, CC-002, CC-003, and CC-005 are complete. CC-004 is blocked until Ryan provides the runtime-permission policy decision; CC-006 is the next ready workstream.

Goal:
Start CC-006 from the course-correction backlog in docs/implementation-plan.md:

- Strengthen contract-reference conformance beyond existence checks where a bounded, testable slice is available.
- Improve reviewer evidence quality without introducing a model-to-model debate loop or changing ADR-002 role routing.
- Revisit gate UX only as a governance-depth improvement; keep the file-and-command gate surface unless the slice explicitly introduces a narrow read-only or reporting command.
- Keep DRH-007 and REF-1..10 recorded as deferred low/refactor material unless the selected CC-006 slice directly needs one.
...
Start with:
Run `git status --short --branch`, preserve unrelated local changes, and select the first active course-correction workstream by order. For CC-006, inspect the existing Check 4 contract-reference implementation, reviewer prompt and critique fixtures, gate-writing and gate-resolution surfaces, and related CLI/check/graph tests before editing. Update docs and tests with any governance-depth behaviour change.
```
