...
| RC-3: Stage-5 Verification And Contract Fidelity | Ready | GAP-011, GAP-012, GAP-013 | Check 4 either runs the promised native validators (schemathesis for OpenAPI; ajv-cli validate-fixtures for JSON Schema; Pydantic model + fixture validation where fixtures exist) or narrows docs/schema claims to the current ref-resolution scope. Check 3, Check 7, and the manifest share one pathspec implementation (git-pathspec, not fnmatch) and one durable `.woof` allow-list that includes `dispositions/story-S<k>.md`. Check 4 surfaces the broken contract artefact path in its `paths` field, not only `EPIC.md`. ajv-cli missing is a preflight failure, not an in-band Check 4 finding. Quality-gate blocking semantics are made consistent across architecture, schema, runner, and tests. | Focused Check 3/4/7 tests, E146 regression expansion, schema validation, package-independent validator probes where applicable, a pathspec-disagreement regression covering staged dispositions; `git diff --check`; `just check`. | `fix(checks): align verification contracts` |
