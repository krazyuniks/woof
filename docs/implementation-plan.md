...
Start with:
The historical phase ledger through Phase 18 `WFL-001` and release-closure workstream RC-1 are complete. The release-closure audit still has active workstreams RC-2 through RC-7. Start with the first `Ready` workstream in the Release-Closure Workstreams table. At the time this prompt was written, that is RC-2: Planning Contract Enforcement.
```
