...
Start with:
The historical phase ledger through Phase 18 `WFL-001` and release-closure workstreams RC-1, RC-2, RC-3, and RC-4 are complete. The release-closure audit still has active workstreams RC-5 through RC-7. Start with the first `Ready` workstream in the Release-Closure Workstreams table. At the time this prompt was written, that is RC-5: Preflight, Bootstrap, And Cartography. RC-5 closes GAP-016 (Claude/Codex auth and model-readiness probes in `woof preflight`), GAP-017 (consumer-owned cartography is either implemented in Woof or demoted to an explicit prerequisite), and GAP-018 (consumer bootstrap can create or print the full `.woof/` starter config and required `.gitignore` entries instead of leaving them hand-assembled).
