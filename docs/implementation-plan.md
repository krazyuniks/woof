...
| GAP-003 | Structured gate decisions drive graph state after human review. | `src/woof/cli/commands/wf.py::_resolve_gate` records the decision and deletes `gate.md`; only `approve` on a `plan_gate` has special behaviour for GitHub plan-summary sync. `plan_gate_resolved()` treats any `gate_resolved` with `gate_type=plan_gate` as resolved regardless of decision. Stale failing `check-result.json` is not cleared on gate resolution. | `schemas/jsonl-events.schema.json` defines a decision enum, but tests only prove `approve` records and deletes a gate. No tests assert `revise_plan`, `split_story`, `abandon_story`, or failed-check re-entry behaviour. | partial | Critical. `revise_plan` can unblock Stage 5 without revising the plan; review gates and failed check gates can reopen from stale result files. | RC-1: Gate Resolution And GitHub State Safety. |
| GAP-004 | Human gates surface the Context block, findings, primary position, and reviewer position through the operator surface. | `human_review_node` returns only `gate open at ...`; generic `write_gate_from_check_result` and `write_gate_for_trigger` auto-position text lacks the architecture's `## Context`, `## Findings`, `## Primary position`, and `## Reviewer position` sections. Plan gates and reviewer-blocker gates do include those sections. | `tests/unit/test_graph.py` asserts plan-gate and blocker-gate headings only. `tests/unit/test_gate_write.py` validates front-matter and event append, not body section completeness. | partial | High. Operators may resolve gates without the role positions the architecture says must be surfaced. | RC-1. |
| GAP-005 | GitHub is the epic authority; no local-only epics are valid. | `woof wf --epic <N>` performs `gh api /rate_limit`, but if `.woof/epics/E<N>/` already exists it runs the graph without fetching the issue, verifying issue existence, or requiring `.last-sync`. | GitHub tests use local epic dirs plus a rate-limit stub. No test rejects an existing local epic with no issue or missing `.last-sync`. | partial | High. A consumer can mutate a local-only epic despite the documented GitHub boundary. | RC-1. |
| GAP-006 | GitHub sync conflict gates offer keep-local, accept-remote, or hand-merge resolution. | `_write_sync_conflict_gate()` writes a plan gate describing those options, but the `woof wf --resolve` enum has no conflict-specific decisions and `_resolve_gate` has no keep-local / accept-remote / hand-merge implementation. | `tests/unit/test_render_epic.py` proves conflict gate creation; no test proves any conflict resolution path. | partial | High. A conflict can be detected but not mechanically resolved through the promised gate contract. | RC-1. |
| GAP-007 | Stage 1 Discovery synthesis has mechanically checked boundary invariants, including non-empty problem framing and ID + deferral reason for every open question. | `discovery_synthesis_node` checks only that four synthesis files exist and are non-empty. There is no discovery synthesis schema or parser for `OPEN_QUESTIONS.md`. | `schemas/planning-node-input.schema.json` names paths only; tests in `tests/unit/test_graph.py` check non-empty files, not open-question structure. | partial | High. Direction-lock artefacts can be malformed while Stage 2 proceeds. | RC-2: Planning Contract Enforcement. |
| GAP-008 | Stage 2 Definition resolves or explicitly carries forward every discovery open question and enforces Definition surface invariants. | `epic_definition_node` validates only `EPIC.md` front-matter against `epic.schema.json`; it does not compare with `OPEN_QUESTIONS.md`. `epic.schema.json` has `open_questions: string[]`, but does not require IDs or resolution metadata. | `tests/unit/test_graph.py` validates schema acceptance; no cross-file open-question tests. | partial | High. Unresolved direction questions can silently enter execution as a locked surface. | RC-2. |
| GAP-009 | Stage 3 Breakdown invariants are enforced before the mandatory plan gate: outcome coverage, CD coverage, dependency closure, no invalid status, and story-scope discipline. | `breakdown_planning_node` validates `plan.json` schema and `epic_id`; cross-artefact invariants live in Check 5 and run during Stage 5 verification, after plan critique and plan gate. | `tests/unit/test_check_5_plan_crossrefs.py` covers invariants in a Stage-5 check; Stage 3 graph tests do not reject cross-ref-invalid plans before the plan gate. | partial | High. Humans can approve a plan before deterministic cross-reference failures are surfaced. | RC-2. |
| GAP-010 | Stage 1-4 planning node output contracts are schema-governed by `planning-node-output.schema.json`. | Runtime `NodeOutput` has no `produced` field and `woof wf --format json` emits the general `node-output` schema, not the planning-node-output contract. | `tests/unit/test_validate.py` fixture-tests `planning-node-output`; graph tests validate runtime outputs against `node-output.schema.json`. | docs-drift | Medium. Consumers cannot rely on the documented planning-node-output shape from the CLI. | RC-2, then RC-7 for docs/schema cleanup. |
| GAP-011 | Stage 5 Check 4 verifies native contract conformance: Schemathesis for OpenAPI, Pydantic model resolution, JSON Schema self-validation and fixtures where present. | `contract_refs.py` verifies OpenAPI file parse + JSON pointer only; it does not run Schemathesis or implementation conformance. JSON Schema refs compile only; there is no fixture validation. Pydantic refs import and verify `BaseModel` subclass. | `tests/unit/test_check_4_contract_refs.py` and E146 fixtures prove reference resolution, not Schemathesis or fixture conformance. `schemas/prerequisites.schema.json` mentions optional `schemathesis`; preflight does not require it based on active contract refs. | partial | Critical. E146-class "right ref, wrong implementation" failures can still pass deterministic checks. | RC-3: Stage-5 Verification And Contract Fidelity. |
| GAP-012 | Stage 5 scope and transaction checks agree on durable `.woof` paths and story pathspec semantics. | Check 7 and the transaction manifest require `.woof/epics/E<N>/dispositions/story-S<k>.md`, but Check 3's allowed `.woof` set omits dispositions. Check 3 uses Git pathspecs; Check 7 and `manifest.py` use `fnmatch`, so complex Git pathspecs can disagree. | `tests/unit/test_check_3_scope.py` does not stage a disposition file in the pass case. `tests/unit/test_graph.py` and Check 7 tests require dispositions. | partial | Critical. Normal Stage-5 transactions can be blocked by contradictory checks, or edge pathspecs can pass one boundary and fail another. | RC-3. |
| GAP-013 | Check 1 quality gates all exit 0 to pass. | `check_1_quality_gates_runner` supports `blocking = false` and returns `ok=True`, `severity="minor"` for non-blocking command failures. | `schemas/quality-gates.schema.json` allows `blocking`, but its top-level and command descriptions still say each command must exit 0. Architecture check table says each gate command exits 0. | docs-drift | Medium. Projects may misunderstand whether advisory quality gates are allowed. | RC-3 or RC-7, depending whether policy or docs changes. |
...
|---|---|---|---|---|---|
| RC-1: Gate Resolution And GitHub State Safety | Ready | GAP-003, GAP-004, GAP-005, GAP-006 | Gate decisions have deterministic state effects; stale failed check results cannot reopen approved gates; conflict gates have supported structured resolutions; existing local epics verify GitHub issue authority or fail loud; operator output surfaces gate context and role positions. | Focused `wf`, gate, GitHub sync, and graph re-entry tests; JSONL/gate schema validation; `git diff --check`; `just check`. | `fix(graph): close gate resolution semantics` |
| RC-2: Planning Contract Enforcement | Ready | GAP-007, GAP-008, GAP-009, GAP-010 | Discovery and Definition boundary checks enforce open-question structure and resolution; Stage 3 runs cross-artefact plan invariants before plan critique/gate; planning node output schema is either emitted by runtime or retired/merged into the general node-output contract. | Focused planning graph, schema, and validation tests; fixtures for malformed discovery/open-question/plan cross-ref states; `git diff --check`; `just check`. | `fix(graph): enforce planning contracts before gate` |
| RC-3: Stage-5 Verification And Contract Fidelity | Ready | GAP-011, GAP-012, GAP-013 | Check 4 either runs the promised native validators or narrows docs/schema claims; Check 3, Check 7, and manifests share one pathspec implementation and durable `.woof` allow-list; quality-gate blocking semantics are made consistent across architecture, schema, runner, and tests. | Focused Check 3/4/7 tests, E146 regression expansion, schema validation, package-independent validator probes where applicable; `git diff --check`; `just check`. | `fix(checks): align verification contracts` |
| RC-4: Audit/Event Durability And Token Accounting | Ready | GAP-014, GAP-015 | Shared atomic write and locked JSONL helpers are used on graph-owned durable writes; event schema vocabulary matches emitters; token usage events are emitted or docs/schema claims are narrowed. | Focused audit, dispatch, graph, and JSONL schema tests including concurrent append/atomic-write fixtures; `git diff --check`; `just check`. | `fix(audit): harden durable event writes` |
...
Start with:
The historical phase ledger through Phase 18 `WFL-001` is complete. The release-closure audit found active workstreams RC-1 through RC-7. Start with the first `Ready` workstream in the Release-Closure Workstreams table. At the time this prompt was written, that is RC-1: Gate Resolution And GitHub State Safety.
```
