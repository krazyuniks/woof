...
Start with:
The historical phase ledger through Phase 18 `WFL-001` and release-closure workstreams RC-1, RC-2, RC-3, RC-4, and RC-5 are complete. The release-closure audit still has active workstreams RC-6 and RC-7. Start with the first `Ready` workstream in the Release-Closure Workstreams table. At the time this prompt was written, that is RC-6: Packaging And Install Portability. RC-6 closes GAP-019 (installed-wheel graph execution must not fail when graph nodes shell back into Woof via `tool_root()/bin/woof`; the source-checkout `uv run --script` wrapper is not safe to execute from an isolated wheel install, so graph subprocesses must use the active Python module/console boundary, and CI or tests must include an installed-package smoke path that exercises graph-owned subprocess calls).
...
RC-4 closed as a docs narrowing on 2026-05-19. The architecture "Atomic writes" line previously described an unimplemented advisory file lock for `epic.jsonl`/`dispatch.jsonl` appends; the WFL-001 per-epic `.wf.lock` plus single-threaded dispatch already serialises the only real writers, and POSIX `O_APPEND` is atomic for line-sized JSONL events. No `_locked_append_jsonl` helper was added. If RC-6..RC-7 surface a new concurrent-writer surface, reopen GAP-014.
