...
Phase A (RC-1..RC-7) is `Completed` as of 2026-05-20. Phase B is unblocked. Phase B work must preserve ADR-001 and ADR-002 invariants. Phase B introduces ADR-003 (issue-tracker abstraction) and may revise architecture sections currently coupling Stage 1 substance and issue-tracker semantics to GitHub.
...
8. Any schema, CLI, graph, dispatch, check runner, gate, tracker, preflight, packaging, or test file directly touched by the selected workstream slice

Goal:
Phase A of the release-closure audit is complete and Phase B RC-B1 and RC-B2 are closed. The next session should pick the first `Ready` workstream in the Phase B (Portability For Arbitrary Consumers) section of docs/implementation-plan.md - at the time this prompt was written that is RC-B3: First-Run Consumer Onboarding. The rule remains: pick a coherent workstream slice large enough to close multiple related child gaps, not a single tiny row. Stop only at a real risky boundary or external blocker.
...
Start with:
The historical phase ledger through Phase 18 `WFL-001`, release-closure workstreams RC-1 through RC-7, and Phase B RC-B1 and RC-B2 are all complete. Phase A of the Release-Closure Audit is closed. The next active backlog is Phase B (Portability For Arbitrary Consumers): BHID-001 (Stage 1 producer skill bundling) closed under RC-B1, BHID-002 (issue-tracker abstraction) closed under RC-B2, and BHID-004 (`bin/woof` portability) was resolved by RC-6, so the remaining open gap is BHID-003 (first-run consumer onboarding). Start with RC-B3 unless the Phase B workstream table records an explicit dependency that overrides the order.

RC-B2 closed on 2026-05-20. `src/woof/cli/github.py` is gone; the issue-tracker abstraction lives in `src/woof/trackers/`: `base.py` (the `Tracker` protocol, `TrackerError`, frozen result records, the conflict trigger/decision constants, and shared filesystem/hash helpers), `epic_body.py` (tracker-agnostic `EPIC.md`<->managed-body rendering and parsing), `github.py` (`GitHubTracker`), `local.py` (`LocalTracker`), and `__init__.py` (the `resolve_tracker` factory). `wf.py`, `main.py`, `preflight.py`, `gate/write.py`, and `transitions.py` depend on the protocol via `resolve_tracker`, not on a provider. `.woof/prerequisites.toml` uses `[tracker]` with `kind = "github" | "local"`. Events `tracker_synced`/`tracker_sync_conflict` are canonical in code and schemas; `github_synced`/`github_sync_conflict` remain legacy enum aliases so pre-RC-B2 `epic.jsonl`/`gate.md` files still validate and resolve. ADR-003 records the boundary and two deviations from the original RC-B2 sketch: conflict detection is folded into the push operations (a standalone `detect_conflict` would double the per-push fetch), and `create_epic`/`assert_epic_authority` were added because `woof wf new` and the GAP-005 authority check need them. The `local` adapter is filesystem-only: `.woof/epics/E<N>/` is self-authoritative, integer epic IDs are allocated as max-existing + 1, push operations are no-ops, and a sync conflict can never arise.

Phase B sequencing: the Phase B Workstreams table is the authority. RC-B3 (complete `woof init` consumer bootstrap, the default cartography-script decision, and a first-run walkthrough in `docs/consumers.md`) is now `Ready` because RC-5, RC-B1, and RC-B2 are complete. RC-B4 (Phase B release evidence) depends on RC-B3. Preserve ADR-001, ADR-002, and ADR-003 invariants: Woof stays graph-led, GPT-5.5 is the preferred primary producer route, Claude Opus 4.7 at `max` effort is the preferred reviewer route, reviewer blockers open human gates, the issue tracker stays behind the `Tracker` protocol, and Woof must not depend on Woof-author-local wrappers or host-specific absolute paths.
```
