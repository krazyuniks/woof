...
| CHK-005 | Completed | Implement Check 5: `check_5_plan_crossrefs`. | Validates plan schema and cross-artefact invariants: outcome refs, contract-decision refs, CD ownership, dependency closure, acyclicity, and status coherence. | Targeted Check 5 runner tests passed: 9 tests; `just check` passed: Ruff lint, Ruff format check, and 120 tests. Registry wiring intentionally untouched for integrator ownership. | `feat(checks): validate plan cross references` |
| CHK-007 | Completed | Implement Check 7: `check_7_commit_transaction`. | Asserts commit readiness: staged diff exists unless gated as empty, required durable `.woof` files are staged, and no unstaged or foreign paths remain. | Focused runner tests passed: 5 tests; `just check` passed: Ruff lint, Ruff format check, and 116 tests. | `feat(checks): verify commit transactions` |
