...
Goal:
Phase A of the release-closure audit is complete. The next session should pick the first `Ready` workstream in the Phase B (Portability For Arbitrary Consumers) section of docs/implementation-plan.md - at the time this prompt was written that is RC-B1: Stage 1 Producer Skill Bundling. The rule remains: pick a coherent workstream slice large enough to close multiple related child gaps, not a single tiny row. Stop only at a real risky boundary or external blocker.
...
Start with:
The historical phase ledger through Phase 18 `WFL-001` and release-closure workstreams RC-1 through RC-7 are all complete. Phase A of the Release-Closure Audit is closed. The next active backlog is Phase B (Portability For Arbitrary Consumers) which covers BHID-001 (Stage 1 producer skill bundling), BHID-002 (issue-tracker abstraction), BHID-003 (first-run consumer onboarding), and BHID-004 (`bin/woof` portability, which was already resolved by RC-6 for the graph-subprocess re-entry path; any remaining BHID-004 surface is operator UX only). Start with RC-B1 unless the Phase B workstream table records an earlier `Ready` slice or an explicit dependency that overrides the order.

RC-7 closed on 2026-05-20 with one docs-only commit. The architecture "Token usage logging" paragraph now states that subprocess dispatch records token usage and that the Python graph itself does not spend tokens, with the `token_usage` enum in `schemas/jsonl-events.schema.json` reserved for a future driver mode. The architecture "Version policy" paragraph no longer references the non-existent `just upgrade-prereqs` recipe; operators upgrade prerequisites manually using the install commands `woof preflight` prints. README has a new "Installed Package Smoke" section pointing at `tests/unit/test_packaging_install.py`, plus an updated source map that names `src/woof/__main__.py` as the install-safe re-entry boundary. Status text in the README reflects RC-7 closure. If a future audit reopens a docs-drift item, reopen the corresponding gap entry rather than starting a new RC-7 slice.

RC-6 closed on 2026-05-19 with commit `7f89562`. `src/woof/__main__.py` plus `_woof_subprocess_argv()` / `_woof_subprocess_env()` in `src/woof/graph/nodes.py` make graph subprocesses install-safe. `tests/unit/test_packaging_install.py` is the released wheel smoke test.

RC-5 closed on 2026-05-19 with three landed changes. Preflight now probes Claude/Codex credential markers (env keys or CLI credential files; documented as best-effort, with live auth and model availability validated only at first dispatch). Cartography is demoted to consumer-owned `./scripts/refresh-cartography`; preflight flags a present-but-non-executable script; architecture text was narrowed. `woof init` scaffolds `.woof/{prerequisites,agents,quality-gates,test-markers}.toml` with `<replace>` placeholders and patches the repo `.gitignore` in an idempotent fenced block. The first-run walkthrough is in `docs/consumers.md`.

Phase B is unblocked. The Phase B Workstreams table is the authority on sequencing inside Phase B. RC-B1 (Stage 1 producer skill bundling) and RC-B2 (issue-tracker abstraction) are independent and may both be `Ready`; pick the first by order unless the table records a dependency. RC-B3 depends on RC-B1 and RC-B2; RC-B4 depends on all three. ADR-003 will be introduced as part of RC-B2.
```
