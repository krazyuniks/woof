...
| ID | Status | Work item | Observable outcomes | Validation | Commit |
|---|---|---|---|---|---|
| WF-001 | Ready | Tighten `woof wf` transition idempotence and crash recovery. | Re-running from the same filesystem state never duplicates durable artefacts; missing transient artefacts are recreated or gated deterministically; crash recovery tests cover at least one mid-transition resume. | Targeted graph tests plus `just check`. | `fix(graph): harden wf crash recovery` |
| WF-002 | Ready | Represent incomplete Stage-5 states as explicit graph states or gates. | Missing required artefacts produce structured failures or `gate.md`; permissive skips are removed from graph-owned execution paths. | Targeted graph/check tests plus `just check`. | `fix(graph): gate incomplete stage states` |
