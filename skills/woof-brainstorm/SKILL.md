...
consumer derive its own fields (Woof story acceptance) from it.
...
checks the schema cannot express. Woof validates the same contracts natively in its own
stacks.

## Handoff

The bundle seeds the project's durable homes (the pm-structure document hierarchy): the design ->
`architecture.md`, the glossary -> `CONTEXT.md`, the decisions -> ADRs, and `work_units[]` -> the
ephemeral per-epic plan the orchestrator consumes. The per-epic plan is deleted at epic close-out via
the promotion gate, which first lands any decisions, target deltas, new terms, and residual work back
into those durable homes.
