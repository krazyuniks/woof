...
def split_front_matter(path: Path) -> tuple[dict, str]:
    """Return (front-matter dict, body prose) from a YAML-front-mattered file."""
    text = path.read_text()
    if not text.startswith("---\n"):
        raise ValueError(f"{path}: no YAML front-matter (file must start with '---\\n')")
    end = text.find("\n---\n", 4)
...
def first_paragraph(prose: str) -> str:
    """First non-empty paragraph of ``prose`` (delimited by a blank line)."""
...
def _deprecation_suffix(item: dict) -> str:
    if not item.get("deprecated"):
        return ""
    rb = item.get("replaced_by")
    return f" _(deprecated → {rb})_" if rb else " _(deprecated)_"


def _contract_ref(cd: dict) -> str:
    if cd.get("openapi_ref"):
        return f"`openapi: {cd['openapi_ref']}`"
    if cd.get("pydantic_ref"):
        return f"`pydantic: {cd['pydantic_ref']}`"
    if cd.get("json_schema_ref"):
        return f"`json_schema: {cd['json_schema_ref']}`"
    return ""


def render_epic_body(front: dict, prose: str, remote_body: str | None = None) -> str:
    """Deterministic transform from EPIC front-matter to gh issue body markdown.

    Preservation rule: if ``remote_body`` contains ``## Observable Outcomes``,
    everything before that heading is preserved as the prefix; everything from
    the heading onward is rewritten. On first push (no remote, or remote
    without the marker), the prefix is the first paragraph of ``prose``.
    """
    out: list[str] = []

    if remote_body and STRUCTURED_HEADING in remote_body:
        prefix, _ = remote_body.split(STRUCTURED_HEADING, 1)
        out.append(prefix.rstrip() + "\n\n")
    else:
        intent = first_paragraph(prose)
        out.append((intent or "_(intent pending)_") + "\n\n")

    out.append("## Observable Outcomes\n\n")
    for o in front["observable_outcomes"]:
        suffix = _deprecation_suffix(o)
        out.append(f"- **{o['id']}** — {o['statement']}{suffix}\n")
        out.append(f"  - Verification: {o['verification']}\n")
    out.append("\n")
    ...
    if cds:
        ...
        out.append("|---|---|---|---|\n")
        for cd in cds:
            related = ", ".join(cd["related_outcomes"])
            title = cd["title"] + _deprecation_suffix(cd)
            out.append(f"| {cd['id']} | {related} | {title} | {_contract_ref(cd)} |\n")
        out.append("\n")

    out.append("## Acceptance Criteria\n\n")
    for c in front["acceptance_criteria"]:
        out.append(f"- {c}\n")
    out.append("\n")
...
def _gh_repo(repo_root: Path) -> str:
    ...
    if not prereq.is_file():
        raise SystemExit(f"woof: {prereq} not found; cannot resolve [github].repo")
    with prereq.open("rb") as fh:
        data = tomllib.load(fh)
    repo = (data.get("github") or {}).get("repo")
    if not repo:
        raise SystemExit(f"woof: {prereq} missing [github].repo")
    return repo


def _gh_fetch_issue(repo: str, number: int) -> dict:
    proc = subprocess.run(
        [
            "gh",
            "api",
    ...
    if proc.returncode != 0:
        raise SystemExit(
            f"woof: gh api /repos/{repo}/issues/{number} failed:\n{proc.stderr.strip()}"
        )
    return json.loads(proc.stdout)


def _gh_edit_body(repo: str, number: int, body_path: Path) -> None:
    proc = subprocess.run(
        ["gh", "issue", "edit", str(number), "--repo", repo, "--body-file", str(body_path)],
        capture_output=True,
        text=True,
    )
    if proc.returncode != 0:
        raise SystemExit(f"woof: gh issue edit {number} failed:\n{proc.stderr.strip()}")


def _last_sync_path(epic_dir: Path) -> Path:
    return epic_dir / ".last-sync"


def _read_last_sync(epic_dir: Path) -> dict | None:
    p = _last_sync_path(epic_dir)
    if not p.is_file():
        return None
    return json.loads(p.read_text())


def _write_last_sync(epic_dir: Path, payload: dict) -> None:
    p = _last_sync_path(epic_dir)
    tmp = p.with_suffix(".last-sync.tmp")
...
def cmd_render_epic(args: argparse.Namespace) -> int:
    ...
    try:
        front, prose = split_front_matter(epic_md)
    ...
    if not args.sync:
        body = render_epic_body(front, prose, remote_body=None)
        if args.output:
    ...
    # --sync: fetch remote, conflict-check, push, update .last-sync
    repo = _gh_repo(repo_root)
    remote = _gh_fetch_issue(repo, args.epic)
    remote_updated_at = remote.get("updated_at") or ""
    remote_body = remote.get("body") or ""

    last_sync = _read_last_sync(epic_dir)
    if last_sync and last_sync.get("updated_at") != remote_updated_at:
        ...
        sys.stderr.write(
            f"woof: github_sync_conflict for E{args.epic}\n"
    ...
    body = render_epic_body(front, prose, remote_body=remote_body)
    with tempfile.NamedTemporaryFile("w", suffix=".md", delete=False, encoding="utf-8") as fh:
        fh.write(body)
        body_path = Path(fh.name)
    try:
        _gh_edit_body(repo, args.epic, body_path)
    finally:
        body_path.unlink(missing_ok=True)

    new_remote = _gh_fetch_issue(repo, args.epic)
    _write_last_sync(
        epic_dir,
        {
            "issue_number": args.epic,
            "updated_at": new_remote.get("updated_at") or "",
