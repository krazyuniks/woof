...
def test_head_and_branch_match_no_drift(tmp_path: Path) -> None:
    ...
    _epic_dir(tmp_path)
    _append_subprocess_returned(tmp_path, head_after=sha, branch_after=branch)
    _write_plan(tmp_path)
    _write_executor_result(tmp_path)
...
def test_head_moved_opens_drift_gate(tmp_path: Path) -> None:
    ...
    _epic_dir(tmp_path)
    _append_subprocess_returned(tmp_path, head_after=sha_at_dispatch, branch_after=branch)
...
def test_branch_switched_opens_drift_gate(tmp_path: Path) -> None:
    ...
    _epic_dir(tmp_path)
    _append_subprocess_returned(tmp_path, head_after=sha, branch_after=original_branch)
